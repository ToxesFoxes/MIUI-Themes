...
def push(src_file, dst_path):
    result = subprocess.call(
        [theme_config.get_adb(), "devices"], stderr=ignore, stdout=ignore)
    if result != 0:
        print(
    ...
    result = subprocess.call(
        [theme_config.get_adb(), "push", src_file, dst_path], stderr=ignore, stdout=ignore)

    if result != 0:
        print_progress_bar(
            progress, 1, suffix=f'Error!' + (" " * 20), length=50)
        print(
            f"failed to push {src_file} to directory {dst_path} with result code {result}\n")
        return result
    if result == 0:
        progress += 1
        print_progress_bar(progress, 1, suffix=f'Complete!' +
                           (" " * 20), length=50)
...
def theme_locks(*locks):
    ...
    for lock in locks:
        ...
        result = subprocess.call(
            [theme_config.get_adb(), "shell", "touch", lock])
        if result != 0:
            return result
    return 0
